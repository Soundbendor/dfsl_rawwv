...
class WeightGenCls(nn.Module):
    def __init__(self, num_classes = 50, dim=512, seed=3, base_idx = [], novel_idx = [], train_phase=TrainPhase.base_init):
        super().__init__()
    ...
    def calc_mask(self, to_mask, include_idxs=[], exclude_idxs=[]):
    ...
    def apply_mask(self, to_mask, cur_mask):
        ret = torch.matmul(to_mask, cur_mask)
        return ret
    ...
    def calc_w_n_plus_1(self, z_arr):
        # z_arr should be (k, dim)
        z_copy = z_arr.detach().clone()
        z_avg = np.mean(z_copy, dim=0) # should be (dim)
        w_att = self.calc_w_att(z_copy) # should be (dim)
        w_n_plus_1 = torch.mul(self.phi_avg, z_avg) + torch.mul(self.phi_att, w_att)
        return w_n_plus_1

    def set_pseudonovel_vec(self, k_novel_idxs, k_novel_sz, k_novel_ft):
        # k_novel_idxs should be of size k_novel
        # k_novel_ft should be of size (all_sizes, dim)
        self.set_train_phase(TrainPhase.base_weightgen)
        cur_mask = self.calc_mask(self.cls_vec, exclude_idxs=k_novel_idxs)
        self.cls_vec_copy = self.apply_mask(self.cls_vec.detach().clone(), cur_mask)
        last_sz = 0
        for i,cur_idx in enumerate(k_novel_idxs):
            cur_sz = k_novel_sz[i]
            cur_wn = self.calc_w_n_plus_1(k_novel_ft[last_sz:cur_sz])
