...
DEF_NEP_API = os.path.join(os.path.split(__file__)[0], "nepapi.txt")
DEF_SEED = 3
...
def runner(model, expr_idx = 0, train_phase = TrainPhase.base_init, seed=DEF_SEED, sr = 16000, max_samp = 118098, max_rng=10000, lr = 1e-4, bs=4, label_smoothing = 0.0, save_dir = DEF_SAVEDIR, res_dir = DEF_RESDIR, data_dir = DEF_DATADIR, epochs=1, save_ivl=0, num_classes_total = 50, use_class_weights = False, to_print=True, to_time = True, to_graph=True, to_res = True, device='cpu', nep=None):
    ...
    if train_phase == TrainPhase.base_init:
        base_init_trainer(model,cur_loss,base_train_data,base_valid_data, expr_idx= expr_idx, epochs=epochs, lr=lr, bs=bs, save_ivl=save_ivl, num_classes_total = num_classes_total, save_dir=save_dir, to_print=to_print, to_time=to_time, to_graph=to_graph, to_res=to_res,nep=nep,device=device)
        tester(model,cur_loss,base_test_data, expr_idx= expr_idx, bs=bs, num_classes_total = num_classes_total, to_print=to_print, to_time=to_time, to_graph=to_graph, to_res=to_res,device=device,pretrain=(train_phase != TrainPhase.base_init),nep=nep)
...
def tester(model, cur_loss, test_data, bs = 4, res_dir = DEF_RESDIR, device='cpu', expr_idx = 0, num_classes_total = 50, to_print = True, to_time = True, to_graph = True, to_res = True, pretrain = False,nep=None):
    ...
    if nep != None:
        nep_batch_parser(nep, res_test,batch_type=BatchType.test, train_phase = TrainPhase.base_init)
...
def base_init_trainer(model, cur_loss, train_data, valid_data, lr=1e-4, bs = 4, epochs = 1, save_ivl=0, save_dir=DEF_SAVEDIR, res_dir = DEF_RESDIR, graph_dir = DEF_GRAPHDIR, device='cpu', expr_idx = 0, num_classes_total = 50, to_print = True, to_time = True, to_graph = True, to_res = True, nep=None):
    ...
    for epoch_idx in range(epochs):
        ...
        if nep != None:
            nep_batch_parser(nep, res_train,batch_type=BatchType.train, train_phase = TrainPhase.base_init)
        ...
        if nep != None:
            nep_batch_parser(nep, res_valid,batch_type=BatchType.valid, train_phase = TrainPhase.base_init)
    if to_graph == True:
...
if __name__ == "__main__":
    ...
    settings_dict = {"expr_idx": expr_idx, "lr": args.lr, "bs": args.bs,
        "epochs": args.epochs, "sr": args.sr,
    ...
    runner(model, train_phase = t_ph,expr_idx = expr_idx, lr=args.lr, bs=args.bs, epochs=args.epochs, save_ivl = args.save_ivl, sr = args.sr, max_samp = max_samp, use_class_weights = args.use_class_weights, num_classes_total = num_classes_total, label_smoothing = args.label_smoothing,
            res_dir=args.res_dir, save_dir=args.save_dir, to_print=args.to_print, to_time=args.to_time, data_dir=args.data_dir, to_graph=args.to_graph, to_res=args.to_res, device=device, nep = nrun)
    if args.to_nep == True:
        nrun.stop()
